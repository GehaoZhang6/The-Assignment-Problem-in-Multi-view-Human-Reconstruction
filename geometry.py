...
def projected_distance(pts_0, pts_1, F):
    ...
    lines = lines.reshape(-1, 19, 1, 3)
    lines = lines.transpose(0, 2, 1, 3)
    points_1 = np.ones([1, pts_1.shape[0], 19, 3])
    points_1[0, :, :, :2] = pts_1
...
def geometry_affinity(points_set, Fs, dimGroup):
    # 获取 points_set 的第一个维度大小，M 代表关键点的数量
    M, _, _ = points_set.shape
    # 初始化距离矩阵 distance_matrix，初始值为 25，形状为 (M, M)
    # 这是为了在计算几何相似性时，如果两个姿态没有计算距离，则保持为默认值 25
    distance_matrix = np.ones((M, M), dtype=np.float32) * 25
    np.fill_diagonal(distance_matrix, 0)

    #import time
    #start_time = time.time()
    # 遍历每个摄像头对 (cam_id0, cam_id1)
    for cam_id0, h in enumerate(range(len(dimGroup) - 1)):
        for cam_add, k in enumerate(range(cam_id0 + 1, len(dimGroup) - 1)):
            ...
            # 如果某个视角没有检测到任何人（即 dimGroup[h] == dimGroup[h + 1]），则跳过该视角
            ...
            # 计算两个视角之间的投影距离，并更新距离矩阵的相应部分
            # 其中 Fs[cam_id0, cam_id1] 是两个摄像头之间的基础矩阵，用于计算投影距离
